...
export default function WageLedger({ eventId, assignments, userLabelMap }: WageLedgerProps) {
  const rows = useMemo(() => {
    ...
    for (const entry of assignments) {
      const current = summary.get(entry.user_id) ?? { totalWage: 0, roleCount: {} };
  ...
  return (
    <section className="rounded-xl border border-emerald-200 bg-emerald-50 p-5 shadow-sm">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <p className="rounded-lg bg-white px-3 py-1 text-sm font-medium text-gray-700">
          Total Budget: <span className="text-ntu-green">NT$ {totalBudget.toLocaleString()}</span>
        </p>
      </div>
